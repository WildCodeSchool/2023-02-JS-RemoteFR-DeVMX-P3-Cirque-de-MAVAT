...
const removeImage = (req, res, next) => {
  ...
  const imageId = Number.parseInt(req.body.imageId, 10);
  const { file } = req.body;
  const schema = joi.object({
    id: joi.number().positive().integer().presence("required"),
    imageId: joi.number().positive().integer().presence("required"),
    file: joi.string().max(255, "utf8").presence("required"),
  });
  const { value, error } = schema.validate({ id, file, imageId });
  if (error) {
